...
const sections = ["starters", "mains", "desserts"];
 ...
 function Item({ name, price, description, image }) {
  ...
  const navigateToDetail = () => {
    // Replace 'DynamicPage' with the name of the dynamic page you want to navigate to.
    try {
      navigation.navigate('Dish', { name, price, description, image });
    } catch (error) {
      console.error('Navigation error:', error);
    }  };
  return (  <Pressable onPress={navigateToDetail}>
...
const Home = ({ navigation }) => {
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        ...
        setProfile(JSON.parse(getProfile) || initialState);
      } catch (e) {
  ...
  const handleFiltersChange = (index) => {
    ...
    updatedSelections[index] = !filterSelections[index];
